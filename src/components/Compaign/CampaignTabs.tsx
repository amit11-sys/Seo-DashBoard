  ...
  const handleCampaignSubmit = async () => {
    ...
    if (!isValid) {
      if (form.formState.errors.name) {
        toast.error(
          form.formState.errors.name.message || "Please provide Name."
        );
      }
      if (form.formState.errors.url) {
        toast.error(
          form.formState.errors.url.message || "Please provide a valid URL."
        );
      }
      return;
    }
    const exists =
      activeCampigns &&
      [...activeCampigns, ...archivedCampigns].find(
        (c: any) => c.projectUrl === values.url
      );
