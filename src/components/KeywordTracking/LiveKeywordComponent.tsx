...
import { getDbKeywordStatusData, getDbLiveKeywordData, getDbLiveKeywordDatabyStatus } from "@/actions/keywordTracking";
...
const LiveKeywordComponent = ({
  ...
  const [campaignLiveKeywordsData, setCampaignLiveKeywordsData] = useState<any>();
  useEffect(() => {
    const fetchDbKeywordData = async () => {
      try {
        const data = await getDbLiveKeywordDatabyStatus(campaignId, campaignStatus);
        setCampaignLiveKeywordsData(data);
      } catch (error) {
  ...
  console.log(campaignLiveKeywordsData,"campaignLiveKeywordsData in component"); 
  useEffect(() => {
    const fetchCampaignStatus = async () => {
      try {
        const dataforCampaignstatus = await getCurrentCampaignIdData(campaignId);
        setCampaignStatus(dataforCampaignstatus?.CurrentCampaignIdData?.status ?? null);
      } catch (error) {
...
useEffect(() => {
  ...
  return (
    <div className="w-full min-h-[80vh]  text-gray-100">
      <Header campaignId={campaignId} campaignStatus={campaignStatus} />
